...
for node in G_2005.nodes():
    weighted_degree = sum(G_2005[node][neighbor]['weight'] for neighbor in G_2005.neighbors(node))
    weighted_degree_centrality[node] = weighted_degree

# 将加权中心性指标写入CSV文件
with open('2005_weighted_centrality_metrics.csv', 'w', newline='') as csvfile:
