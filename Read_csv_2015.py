...
np.set_printoptions(threshold=np.inf)  # 设置print阈值为正无穷，不会省略输出内容，便于检查核对

# 指定CSV文件路径
# csv_file_1995 = '1995_io95a294_94.CSV'
# csv_file_2005 = '2005_io05a301_108.CSV'
...
for node in G_2015.nodes():
    weighted_degree = sum(G_2015[node][neighbor]['weight'] for neighbor in G_2015.neighbors(node))
    weighted_degree_centrality[node] = weighted_degree

# 将加权中心性指标写入CSV文件
with open('2015_weighted_centrality_metrics.csv', 'w', newline='') as csvfile:
