...
# 指定CSV文件路径
csv_file_1995 = '1995_io95a232_32.xls.CSV'
...
for node in G_1995.nodes():
    weighted_degree = sum(G_1995[node][neighbor]['weight'] for neighbor in G_1995.neighbors(node))
    weighted_degree_centrality[node] = weighted_degree


# 将加权中心性指标写入CSV文件
with open('1995_weighted_centrality_metrics.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
